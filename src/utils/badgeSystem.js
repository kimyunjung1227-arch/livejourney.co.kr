    ...
    .forEach(([region, cntRaw]) => {
      ...
      mk(
        `region:${region}:tier${st.tier}`,
        {
          ...
          description:
            st.tier === 1
              ? `${region} 지역의 매력을 발견하기 시작한 첫 발걸음. 현장의 실시간 공기를 커뮤니티에 처음으로 공유하며 여정을 시작한 분들에게 수여됩니다.`
              : st.tier === 2
                ? `${region} 지역의 생생한 진실을 다른 여행자들에게 연결하는 가교. 정보의 시차를 줄이는 실시간 업데이트를 통해 많은 이들의 실패 없는 여행을 돕고 있습니다.`
                : `${region} 지역에 관한 한 가장 신뢰할 수 있는 현장 가이드. 높은 신뢰지수를 바탕으로 지역의 구석구석을 실시간으로 꿰뚫고 있어, 이 지역 여행자들에게 없어서는 안 될 필수적인 지표가 된 분입니다.`,
          icon: '🧭',
          category: '지역 테마',
          difficulty: st.tier,
